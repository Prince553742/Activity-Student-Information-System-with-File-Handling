...
def validation():
    while True:
        ...
        if studentID.isdigit() and len(studentID) == 4: 
            return studentID
        else: 
            print("Invalid Student ID, Only 4 digit")
...
def SaveToFile():
    with open("Catayoc.txt", "w") as prince: 
        for sid, details in students.items():
            name = details["info"][1]
            age = details["age"]
            grades = ",".join(map(str, details["grades"]))  
            prince.write(f"{sid}|{name}|{age}|{grades}\n")
    print("Student data saved to Database")

def LoadFromFile():
    if not os.path.exists("Catayoc.txt"):
        print("No saved file found")
        return
    with open("Catayoc.txt", "r") as f:
        for line in f:
    ...
    print("Student data loaded from Catayoc.txt")

while True:
    choice = int(input("\n[1] Add Students \n[2] Display all Students \n[3] Update student Information \n[4] Delete a student Record" \
    "\n[5] Save student data to File \n[6] Load student data back from the File \n[7] Exit \nAnswer: "))
    if choice == 1: AddStudent()
    elif choice == 2: DisplayStudent()
    elif choice == 3: UpdateStudent()
    elif choice == 4: DeleteStudent()
    elif choice == 5: SaveToFile()
    elif choice == 6: LoadFromFile()
    elif choice == 7: break
